...
Public Class WebService_LedgerMaster
    ...
    Dim UserGBLCompanyID As String = ""
    ...
    Public Function MasterGrid(ByVal masterID As String) As String
        ...
        Else
            str2 = "Select nullif(SelectQuery,'') as SelectQuery From LedgerGroupMaster Where LedgerGroupID='" & masterID & "' and isnull(IsDeletedTransaction,0)<>1"
        End If
    ...
    Public Function MasterGridColumnHide(ByVal masterID As String) As String
        ...
        Else
            str = "SELECT  nullif(GridColumnHide,'') as GridColumnHide,nullif(TabName,'') as TabName,nullif(ConcernPerson,'') as ConcernPerson,nullif(EmployeeMachineAllocation,'') as EmployeeMachineAllocation FROM LedgerGroupMaster Where LedgerGroupID= '" & masterID & "' and isnull(IsDeletedTransaction,0)<>1"
        End If
    ...
    Public Function MasterGridColumn(ByVal masterID As String) As String
        ...
        Else
            str = "select nullif(GridColumnName,'') as GridColumnName From LedgerGroupMaster Where LedgerGroupID='" & masterID & "' and isnull(IsDeletedTransaction,0)<>1 "
        End If
    ...
    Public Function SaveData(ByVal CostingDataLedgerMaster As Object, ByVal CostingDataLedgerDetailMaster As Object, ByVal MasterName As String, ByVal ActiveLedger As String, ByVal LedgerGroupID As String, ByVal LedgerRefCode As String) As String
        ...
        Try
            ...
            Else
                LedgerCodestr2 = "select nullif(LedgerGroupPrefix,'') as LedgerGroupPrefix from  LedgerGroupMaster Where LedgerGroupID='" & LedgerGroupID & "' and isnull(IsDeletedTransaction,0)<>1"
            End If
            ...
            If db.GetColumnValue("CanSave", "UserSubModuleAuthentication", " UserID=" & GBLUserID & " And LedgerGroupID=" & LedgerGroupID) = False Then Return "You are not authorized to save..!"

            str = "Select top(1) Nullif(LedgerRefCode,'') as LedgerRefCode from LedgerMaster where LedgerRefCode = '" & LedgerRefCode & "'  And Isnull(IsDeletedTransaction,0) = 0"
            ...
            Else
                db.ExecuteNonSQLQuery("EXEC UpdateLedgerMasterValuesMultiUnit " & GBLCompanyID & "," & LedgerID)
            End If
            KeyField = "Success"
    ...
    Public Function UpdateData(ByVal CostingDataLedgerMaster As Object, ByVal CostingDataLedgerDetailMaster As Object, ByVal MasterName As String, ByVal LedgerID As String, ByVal UnderGroupID As String, ByVal ActiveLedger As String, ByVal LedgerRefCode As String) As String
        ...
        GBLFYear = Convert.ToString(HttpContext.Current.Session("FYear"))
        DBType = Convert.ToString(HttpContext.Current.Session("DBType"))
        UserGBLCompanyID = Convert.ToString(HttpContext.Current.Session("UserCompanyID"))
        ...
        If db.GetColumnValue("CanEdit", "UserSubModuleAuthentication", " UserID=" & GBLUserID & " And LedgerGroupID=" & UnderGroupID & "") = False Then Return "You are not authorized to update..!"
        ...
        Try
            ...
            End If
            wherecndtn = "LedgerID=" & LedgerID & " And LedgerGroupID=" & UnderGroupID & ""
            ...
            Else
                str2 = "Select nullif(ParentFieldValue,'') as ParentFieldValue From LedgerMasterDetails Where ParentFieldName='ISLedgerActive' and FieldName='ISLedgerActive' and LedgerID='" & LedgerID & "'  and LedgerGroupID='" & UnderGroupID & "' and isnull(IsDeletedTransaction,0)<>1"
            End If
            ...
            Else
                ...
                Else
                    ...
                    ActiveLedgerUpdateMaster = "Update LedgerMaster Set ModifiedDate=GetDate(),UserID=" & GBLUserID & ",CompanyID=" & GBLCompanyID & ",ModifiedBy='" & GBLUserID & "',ISLedgerActive='" & ActiveLedger & "'  Where  LedgerID='" & LedgerID & "'  and LedgerGroupID='" & UnderGroupID & "' "
                    db.ExecuteNonSQLQuery(ActiveLedgerUpdateMaster)
                End If
    ...
    Public Function DeleteData(ByVal txtGetGridRow As String, ByVal MasterName As String, ByVal UnderGroupID As String) As String
        ...
        Try
            ...
            If DBType = "MYSQL" Then
                ...
                db.ExecuteNonSQLQuery(str)
            Else
                str = "Update LedgerMasterDetails Set DeletedBy='" & GBLUserID & "',DeletedDate=GetDate(),IsDeletedTransaction=1  WHERE LedgerID='" & txtGetGridRow & "' And LedgerGroupID='" & UnderGroupID & "' "
                db.ExecuteNonSQLQuery(str)

                str = ""
                str = "Update LedgerMaster Set DeletedBy='" & GBLUserID & "',DeletedDate=GetDate(),IsDeletedTransaction=1  WHERE LedgerID='" & txtGetGridRow & "' And LedgerGroupID='" & UnderGroupID & "'  "
                db.ExecuteNonSQLQuery(str)
                str = ""
                str = "Update BusinessVerticalDetails Set DeletedBy='" & GBLUserID & "',DeletedDate=GetDate(),IsDeletedTransaction=1  WHERE CompanyID='" & GBLCompanyID & "' and LedgerID='" & txtGetGridRow & "'"
                db.ExecuteNonSQLQuery(str)
                str = ""
                str = "Update EmbargoDetails Set DeletedBy='" & GBLUserID & "',DeletedDate=GetDate(),IsDeletedTransaction=1  WHERE CompanyID='" & GBLCompanyID & "' and LedgerID='" & txtGetGridRow & "'"
                db.ExecuteNonSQLQuery(str)
            End If
    ...
    Public Function CheckPermission(ByVal LedgerID As String) As String
        ...
        Try
            ...
            Dim s2 As String = "ConcernPersonMaster,ClientMachineCostSettings,SupplierWisePurchaseSetting"
            Dim list As String() = s.Split(",")
            Dim list2 As String() = s.Split(",")
            For i = 0 To list.Length - 1
                distblName = ""
                distblName = list(i)
            ...
            For j = 0 To list2.Length - 1
                ...
                str3 = ""
                str3 = "select isnull(LedgerID,0) LedgerID from " & distblName & " Where LedgerID=" & LedgerID & " and isnull(IsDeletedTransaction,0)<>1"
                db.FillDataTable(dtNew, str3)
                Dim E As Integer = dtNew.Rows.Count
                If E > 0 Then
                    KeyField = "Exist"
    ...
    Public Function MasterGridLoadedData(ByVal masterID As String, ByVal Ledgerid As String) As String
        ...
        Else
            selQ = "Execute SelectedRowLedgerMultiUnit  '',"
            str = selQ & masterID & "," & Ledgerid
        End If
        db.FillDataTable(dataTable, str)
        data.Message = ConvertDataTableTojSonString(dataTable)
    ...
    Public Function DrillDownMasterGrid(ByVal masterID As String, ByVal TabID As String, ByVal LedgerID As Integer) As String
        ...
        Else
            str2 = "Select SelectQuery From LedgerMasterDrilDown Where IsNull(SelectQuery,'')<>'' And IsDeletedTransaction=0 And LedgerGroupID='" & masterID & "' and TabName='" & TabID & "' and isnull(IsDeletedTransaction,0)<>1"
        End If
        db.FillDataTable(dt, str2)
        Dim i As Integer = dt.Rows.Count
        If i > 0 Then
    ...
    Public Function SelectBoxLoad(ByVal Qery As String, ByVal selID As String) As String
        ...
        Try
            ...
            For i = 0 To NewQuery.Length - 1
                ...
                Else
                    str3 = "Select nullif(SelectboxQueryDB,'') as SelectboxQueryDB From LedgerGroupFieldMaster Where LedgerGroupFieldID=" & QS & " and isnull(IsDeletedTransaction,0)<>1"
                End If
    ...
    Public Function ConvertLedgerToConsignee(ByVal LedID As Integer) As String
        ...
        Try
            ...
            Else
                LedgerCodestr2 = "Select nullif(LedgerGroupPrefix,'') As Prefix,LedgerGroupID from  LedgerGroupMaster Where Isnull(IsDeletedTransaction,0)=0 And LedgerGroupID=(Select LedgerGroupID From LedgerGroupMaster Where LedgerGroupNameID=44 And Isnull(IsDeletedTransaction,0)=0"
            End If
            db.FillDataTable(DTLedgerCode, LedgerCodestr2)
            If DTLedgerCode.Rows.Count > 0 Then
                ...
                str = "Select LedgerID,LedgerName From LedgerMaster Where LedgerGroupID=(Select LedgerGroupID From LedgerGroupMaster Where LedgerGroupNameID=24)  And IsDeletedTransaction=0 And LedgerName Not In (Select Distinct LedgerName From LedgerMaster Where IsDeletedTransaction=0 And LedgerGroupID=" & LedgerGroupID & ")"
                ...
                If DTConsignee.Rows.Count > 0 Then
                    ...
                    Else
                        ...
                        For i = 0 To DTConsignee.Rows.Count - 1
                            ...
                            LedgerCode = db.GeneratePrefixedNo("LedgerMaster", LedgerCodePrefix, "MaxLedgerNo", MaxLedgerID, "", " Where LedgerCodeprefix='" & LedgerCodePrefix & "' And isnull(IsDeletedTransaction,0)=0")
                            ...
                            Using updateTransaction As New Transactions.TransactionScope
                                ...
                                Dim DtDetails As New DataTable
                                str = "SELECT ParentLedgerID, ParentFieldName, ParentFieldValue," & NewLedgerID & " As LedgerID, FieldID, FieldName, FieldValue, SequenceNo," & LedgerGroupID & " As LedgerGroupID," & GBLCompanyID & " As CompanyID," & GBLUserID & " As UserID, Getdate() As CreatedDate,'" & GBLFYear & "' As FYear FROM LedgerMasterDetails Where FieldName IN(Select Distinct FieldName From LedgerGroupFieldMaster Where LedgerGroupID=" & LedgerGroupID & ") And (LedgerID = " & LedID & ") "
                                db.FillDataTable(DtDetails, str)
                                str = db.InsertSecondaryDataJobCard(DtDetails, "LedgerMasterDetails", "CreatedBy", GBLUserID, "", "", "")
                                If str <> "200" Then
                                    con.Close()
    ...
    Public Function GetLedgerName(ByVal TabelID As String) As String
        ...
        Else
            str = "execute [GetFilteredLedgerMasterDataMultiUnit] '','" & TabelID & "',' And LedgerID In (Select Distinct LedgerID From LedgerMasterDetails Where FieldName=''ISLedgerActive'' And FieldValue=''True'')'"
        End If
    ...
    Public Function GetExistCocrnPerson() As String
        ...
        Else
            str = "select distinct ConcernPersonID,nullif(LedgerID,'') as LedgerID,nullif(Name,'') as Name,nullif(Address1,'') as Address1,nullif(Address2,'') as Address2,nullif(Mobile,'') as Mobile,nullif(Email,'') as Email,nullif(Designation,'') as Designation ,nullif(IsPrimaryConcernPerson,'') as IsPrimaryConcernPerson from ConcernPersonMaster where isnull(IsDeletedTransaction,0)<>1"
        End If
    ...
    Public Function DeleteConcernPersonData(ByVal LedgerID As String) As String
        ...
        Try
            ...
            Else
                str = "Update ConcernPersonMaster Set DeletedBy='" & GBLUserID & "',DeletedDate=GetDate(),IsDeletedTransaction=1  WHERE LedgerID='" & LedgerID & "'"
            End If
            db.ExecuteNonSQLQuery(str)
            KeyField = "Success"
        Catch ex As Exception
    ...
    Public Function DeleteConcernPersonDataGridRow(ByVal ConcernPersonID As String, ByVal LedgerID As String) As String
        ...
        Try
            ...
            db.ExecuteNonSQLQuery("Delete from ConcernPersonMaster WHERE ConcernPersonID='" & ConcernPersonID & "' And LedgerID='" & LedgerID & "' ")
    ...
    Public Function GetOperatorName() As String
        ...
        Else
            str = "Select Isnull(L.LedgerID,0) As LedgerID,nullif(L.LedgerName,'') As LedgerName From LedgerMaster As L Inner Join LedgerMasterDetails As LD On L.LedgerID=LD.LedgerID And L.CompanyID=LD.CompanyID INNER JOIN ProductionUnitMaster As PUM ON PUM.ProductionUnitID = L.ProductionUnitID Where LD.FieldName='Designation' And Upper(LD.FieldValue)='OPERATOR' And Isnull(L.IsDeletedTransaction,0)<>1 And L.ProductionUnitID = '" & ProductionUnitID & "' "
        End If
    ...
    Public Function GetEmployeeName(ByVal TabelID As String) As String
        ...
        Else
            str = "select nullif(LedgerID,'') as LedgerID,nullif(LedgerName,'') as LedgerName from LedgerMaster where LedgerGroupID='" & TabelID & "' and isnull(IsDeletedTransaction,0)<>1"
        End If
    ...
    Public Function SaveEmpMachineAllocation(ByVal CostingDataMachinAllocation As Object, ByVal EmployeID As String, ByVal GridRow As String) As String
        ...
        Try
            ...
            If GridRow <> "" Then
                ...
                AddColName = "ModifiedDate,CreatedDate,UserID,CompanyID,FYear,CreatedBy,ModifiedBy,MachineIDString,EmployeeMachineAllocationID,ProductionUnitID"
                AddColValue = "GetDate(),GetDate(),'" & GBLUserID & "','" & GBLCompanyID & "','" & GBLFYear & "','" & GBLUserID & "','" & GBLUserID & "','" & GridRow & "','" & maxValue & "','" & ProductionUnitID & "'"
    ...
    Public Function ExistMachineID(ByVal EmployeeID As String) As String
        ...
        Else
            str = "select top(1) nullif(MachineIDString,'') as MachineIDString from EmployeeMachineAllocation where ProductionUnitID ='" & ProductionUnitID & "' and LedgerID='" + EmployeeID + "' And isnull(IsDeletedTransaction,0)<>1"
        End If
    ...
    Public Function DeleteEmpMacineAllo(ByVal LedgerID As String) As String
        ...
        Try
            ...
            Else
                str = "Update EmployeeMachineAllocation Set ModifiedBy='" & GBLUserID & "',DeletedBy='" & GBLUserID & "',DeletedDate=GetDate(),ModifiedDate=GetDate(),IsDeletedTransaction=1  WHERE LedgerID='" & LedgerID & "'"
            End If
            db.ExecuteNonSQLQuery(str)
    ...
    Private Function DuplicateLedgerGroupValidate(ByVal TabelID As String, ByVal tblObj As Object) As Boolean
        ...
        Try
            ...
            Else
                str2 = "Select nullif(SaveAsString,'') as SaveAsString From LedgerGroupMaster Where LedgerGroupID='" & TabelID & "' and Isnull(IsDeletedTransaction,0)<>1"
            End If
            db.FillDataTable(dtExist, str2)
    ...
    Public Function GetLedgerGroupNameID(ByVal MID As String) As String
        ...
        str = "select LedgerGroupNameID from LedgerGroupMaster where LedgerGroupID='" & MID & "' "
    ...
    Public Function GetSupplierName(ByVal LedgerGrNID As String) As String
        ...
        Else
            str = "Select Isnull(LedgerID,0) AS LedgerID,Nullif(LedgerName,'') AS LedgerName From LedgerMaster Where CompanyID='" & GBLCompanyID & "' AND Isnull(IsDeletedTransaction,0)<>1 " &
                " And LedgerGroupID In(Select Distinct LedgerGroupID From LedgerGroupMaster Where LedgerGroupNameID ='" & LedgerGrNID & "') Order By Nullif(LedgerName,'')"
        End If
    ...
    Public Function GroupGrid() As String
        ...
        Else
            str = "Select Isnull(ItemGroupID,0) AS ItemGroupID,Nullif(ItemGroupName,'') AS ItemGroupName From ItemGroupMaster Where Isnull(IsDeletedTransaction,0)<>1 Order By Nullif(ItemGroupName,'')"
        End If
    ...
    Public Function SpareGroupGrid() As String
        ...
        str = "Select Distinct SparePartGroup from SparePartMaster Where IsDeletedTransaction=0 Order By SparePartGroup"
    ...
    Public Function ExistGroupID(ByVal SupplierID As String) As String
        ...
        Else
            str = "select top(1) nullif(GroupAllocationIDString,'') as GroupAllocationIDString from SupplierItemGroupAllocation where LedgerID='" + SupplierID + "' And isnull(IsDeletedTransaction,0)<>1"
        End If
    ...
    Public Function SaveGroupAllocation(ByVal CostingDataGroupAllocation As Object, ByVal SuppID As String, ByVal GridRow As String, ByVal ObjSparePartAllocation As Object) As String
        ...
        Try
            ...
            Else
                db.ExecuteNonSQLQuery("Delete from SupplierItemGroupAllocation WHERE  LedgerID='" & SuppID & "' and isnull(IsDeletedTransaction,0)<>1")
                db.ExecuteNonSQLQuery("Delete from SupplierSpareGroupAllocation WHERE LedgerID='" & SuppID & "' and isnull(IsDeletedTransaction,0)<>1")
            End If
    ...
    Public Function DeleteGroupAllo(ByVal LedgerID As String) As String
        ...
        Try
            ...
            Else
                str = "Update SupplierItemGroupAllocation Set DeletedBy='" & GBLUserID & "',DeletedDate=GetDate(),IsDeletedTransaction=1  WHERE LedgerID='" & LedgerID & "'"
            End If
            db.ExecuteNonSQLQuery(str)
    ...
    Public Function ExistSparesGroupID(ByVal SupplierID As String) As String
        ...
        Else
            str = "Select STUFF((SELECT DISTINCT ','+(c.SparePartGroup) FROM [dbo].[SupplierSpareGroupAllocation] c Where c.LedgerID=" + SupplierID + " And c.IsDeletedTransaction=0 FOR XML PATH(''), TYPE ).value('.', 'nvarchar(max)'), 1, 1, '') As IDString;"
        End If
    ...
    '---------------Close Master code---------------------------------
    '---------------Open AdditionSettingData ---------------------------------
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(ResponseFormat:=ResponseFormat.Json)>
    Public Function GetAdditionlSettingdata() As String

        GBLCompanyID = Convert.ToString(HttpContext.Current.Session("CompanyID"))
        DBType = Convert.ToString(HttpContext.Current.Session("DBType"))
        Dim SalesPersondataTable As New DataTable
        Dim BusinessVerticalDataTable As New DataTable

        Dim str1 As String = "Select isnull(LedgerID,0) as LedgerID,nullif(LedgerName,'') as LedgerName from LedgerMaster where DepartmentID = -50 and CompanyID = '" & GBLCompanyID & "' and isnull(IsDeletedTransaction,0) <>1"
        Dim str2 As String = "Select isnull(BusinessVerticalID,0) as BusinessVerticalID,nullif(BusinessVerticalName,'') as BusinessVerticalName from ClientBusinessVerticalMaster where CompanyID = '" & GBLCompanyID & "' and isnull(IsDeletedTransaction,0) <>1"

        db.FillDataTable(BusinessVerticalDataTable, str2)
        Dim BusinessVerticalData As String = db.ConvertDataTableTojSonString(BusinessVerticalDataTable)
        db.FillDataTable(SalesPersondataTable, str1)
        Dim SalesPersondata As String = db.ConvertDataTableTojSonString(SalesPersondataTable)

        BusinessVerticalDataTable.TableName = "BusinessVerticalData"
        SalesPersondataTable.TableName = "SalesPersondata"
        Dim dataset As New DataSet
        dataset.Merge(BusinessVerticalDataTable)
        dataset.Merge(SalesPersondataTable)
        data.Message = db.ConvertDataSetsTojSonString(dataset)
        js.MaxJsonLength = 2147483647
        Return js.Serialize(data.Message)
    End Function

    ''----------------------------Open BusinessCerticalDetails  Save Data  ------------------------------------------
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(ResponseFormat:=ResponseFormat.Json)>
    Public Function SaveBusinessVerticalDetailsData(ByVal BusinessVerticalDetailsData As Object) As String

        Dim dt As New DataTable
        Dim KeyField, BusinessVerticalDeatilID As String
        Dim AddColName, AddColValue, TableName As String
        ...
        Try
            If db.CheckAuthories("LedgerMaster.aspx", GBLUserID, GBLCompanyID, "CanSave") = False Then Return "You are not authorized to Save"
            TableName = "BusinessVerticalDetails"
            ...
            End If
            BusinessVerticalDeatilID = db.InsertDatatableToDatabase(BusinessVerticalDetailsData, TableName, AddColName, AddColValue)

            If IsNumeric(BusinessVerticalDeatilID) = False Then
                Return "fail" & BusinessVerticalDeatilID
            End If
            KeyField = "Success"
        Catch ex As Exception
            KeyField = "fail"
        ...
        Return KeyField
    End Function
    ''----------------------------Open BusinessCerticalDetails  Update Data  ------------------------------------------
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(ResponseFormat:=ResponseFormat.Json)>
    Public Function UpdateBusinessVerticalDetailsData(ByVal BusinessVerticalDetailsData As Object, ByVal BusinessVerticalDetailID As String) As String
        ...
        GBLFYear = Convert.ToString(HttpContext.Current.Session("FYear"))
        DBType = Convert.ToString(HttpContext.Current.Session("DBType"))
        If db.CheckAuthories("LedgerMaster.aspx", GBLUserID, GBLCompanyID, "CanEdit") = False Then Return "You are not authorized to Edit"
        ...
        Try
            ...
            Else
                AddColName = "ModifiedDate=GetDate(),UserID=" & GBLUserID & ",CompanyID=" & GBLCompanyID & ",ModifiedBy='" & GBLUserID & "'"
            End If
            wherecndtn = "CompanyID=" & GBLCompanyID & " And BusinessVerticalDetailID=" & BusinessVerticalDetailID & ""
            db.UpdateDatatableToDatabase(BusinessVerticalDetailsData, TableName, AddColName, 0, wherecndtn)
            KeyField = "Success"
    ...
    ''----------------------------Open BusinessCerticalDetails Delete  Data  ------------------------------------------
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(ResponseFormat:=ResponseFormat.Json)>
    Public Function DeleteBusinessVerticalDetailData(ByVal BusinessVerticalDetailID As String) As String
        Dim KeyField As String
        Dim Dt As New DataTable()
        ...
        GBLFYear = Convert.ToString(HttpContext.Current.Session("FYear"))
        DBType = Convert.ToString(HttpContext.Current.Session("DBType"))
        If db.CheckAuthories("LedgerMaster.aspx", GBLUserID, GBLCompanyID, "CanDelete") = False Then Return "You are not authorized to Edit"

        Try
            str = "Update BusinessVerticalDetails Set DeletedBy=" & GBLUserID & ",DeletedDate=GETDATE(),IsDeletedTransaction = 1 WHERE CompanyID=" & GBLCompanyID & " and BusinessVerticalDetailID='" & BusinessVerticalDetailID & "'"
            db.ExecuteNonSQLQuery(str)
            KeyField = "Success"
        Catch ex As Exception
            KeyField = "Error: " & ex.Message
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(ResponseFormat:=ResponseFormat.Json)>
    Public Function GetBusinessVerticalDetaildata(ByVal LedgerID As String, ByVal BusinessVerticalID As String) As String
        Dim dataTable1 As New DataTable

        GBLCompanyID = Convert.ToString(HttpContext.Current.Session("CompanyID"))
        DBType = Convert.ToString(HttpContext.Current.Session("DBType"))
        Dim str As String = "Select isnull(BVD.BusinessVerticalDetailID,0) as BusinessVerticalDetailID,isnull(BVD.LedgerID,0) as LedgerID,isnull(BVD.SalesPersonLedgerID,0) as SalesPersonLedgerID,isnull(BVD.BusinessVerticalID,0) as BusinessVerticalID,isnull(BVD.MaxCreditPeriod,0) as MaxCreditPeriod,Isnull(BVD.MaxCreditLimit,0) as MaxCreditLimit,Isnull(BVD.MaxFixedLimit,0) as MaxFixedLimit,Isnull(BVD.TotalAvailableCredit,0) as TotalAvailableCredit,Isnull(BVD.MaxOverdueDays,0) as MaxOverdueDays,Isnull(BVD.TotalOverdueAmount,0) as TotalOverdueAmount,Isnull(BVD.LastYearExposure,0) as LastYearExposure,nullif(BVD.Status,'') as Status,nullif(BVD.EmbargoStatus,'') as EmbargoStatus,nullif(LM.LedgerName,'') as LedgerName,nullif(SP.LedgerName,'') as SalesPersonName,nullif(CBVM.BusinessVerticalName,'') as BusinessVerticalName from BusinessVerticalDetails as BVD Inner Join LedgerMaster as LM on LM.LedgerID = BVD.LedgerID and isnull(LM.IsDeletedTransaction,0) <> 1Inner Join ClientBusinessVerticalMaster as CBVM on CBVM.BusinessVerticalID = BVD.BusinessVerticalID and isnull(CBVM.IsDeletedTransaction,0) <> 1 Inner Join LedgerMaster as SP on SP.LedgerID = BVD.SalesPersonLedgerID and isnull(SP.IsDeletedTransaction,0) <> 1 and SP.DepartmentID = -50 where BVD.CompanyID = '" & GBLCompanyID & "' and BVD.LedgerID = '" & LedgerID & "' and isnull(BVD.IsDeletedTransaction,0) <> 1"
        Dim str1 As String = "Select Sum(Isnull(ID.BasicAmount,0)) As TotalSale, IM.LedgerID  From InvoiceTRansactionDetail as ID INNER JOIN InvoiceTransactionMain as IM ON  IM.InvoiceTransactionID = ID.InvoiceTransactionID INNER JOIN LedgerMaster as LM ON LM.LedgerID = IM.LedgerID Inner Join BusinessVerticalDetails as BVD on BVD.LedgerID = LM.LedgerID and Isnull(BVD.IsDeletedTransaction,0) = 0 and Isnull(IM.IsDeletedTransaction,0) = 0 and Isnull(ID.IsDeletedTransaction,0) = 0 Where    LM.LedgerID = '" & LedgerID & "'  and BVD.BusinessVerticalID = '" & BusinessVerticalID & "' and IM.CompanyID = '" & GBLCompanyID & "' Group BY IM.LedgerID"
        db.FillDataTable(dataTable, str)
        Dim Embargodata As String = db.ConvertDataTableTojSonString(dataTable)
        db.FillDataTable(dataTable1, str1)
        Dim Statusdata As String = db.ConvertDataTableTojSonString(dataTable1)
        dataTable.TableName = "Result1"
        dataTable1.TableName = "Result2"

        Dim dataset As New DataSet
        dataset.Merge(dataTable)
        dataset.Merge(dataTable1)
    ...
    ''----------------------------Open EmbargoDetails  Save Data  ------------------------------------------
    '<WebMethod(EnableSession:=True)>
    '<ScriptMethod(ResponseFormat:=ResponseFormat.Json)>
    'Public Function SaveEmbargoDetailsData(ByVal EmbargoDetailsData As Object, ByVal BusinessVerticalID As String, ByVal LedgerID As String, ByVal EmbargoStatus As String, ByVal txtStatus As String) As String

    '    Dim dt, embargodata As New DataTable
    '    Dim KeyField, EmbargoID As String
    '    Dim AddColName, AddColValue, TableName As String
    ...
    '    Try
    '        If db.CheckAuthories("LedgerMaster.aspx", GBLUserID, GBLCompanyID, "CanEdit") = False Then Return "You are not authorized to Save"
    '        Dim Embargo = "SELECT isnull(Embargo,0) as Embargo FROM ADHOCLevelApprovalSettings WHERE UserID ='" & GBLUserID & "' and ISNULL(IsDeletedTransaction, 0) = 0 AND CompanyID = '" & GBLCompanyID & "'"
    '        db.FillDataTable(embargodata, Embargo)


    '        If embargodata.Rows.Count > 0 Then
    '            If Convert.ToBoolean(embargodata.Rows(0)("Embargo")) = False Then
    ...
    '            AddColValue = "GetDate(),GetDate(),'" & GBLUserID & "','" & GBLCompanyID & "','" & GBLFYear & "','" & GBLUserID & "','" & GBLUserID & "'"
    '        End If
    '        EmbargoID = db.InsertDatatableToDatabase(EmbargoDetailsData, TableName, AddColName, AddColValue)
    '        If IsNumeric(EmbargoID) = False Then
    '            Return "fail" & EmbargoID
    '        End If
    '        Dim str As String = "UPDATE BusinessVerticalDetails SET EmbargoStatus = '" & EmbargoStatus & "' ,Status = '" & txtStatus & "' WHERE CompanyID = " & GBLCompanyID & "  AND LedgerID = '" & LedgerID & "'"
    '        db.ExecuteNonSQLQuery(str)
    '        KeyField = "Success"
    '    Catch ex As Exception
    '        KeyField = "fail"
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(ResponseFormat:=ResponseFormat.Json)>
    Public Function SaveDataPlacedEmbargo(ByVal ObjMainData As Object, ByVal Arrdata As Object) As String
        ...
        Try
            'If db.CheckAuthories("PlaceEmbargo.aspx", GBLUserID, GBLCompanyID, "CanSave") = False Then Return "You are not authorized to Save"
            'Dim Embargo = "SELECT isnull(Embargo,0) as Embargo FROM ADHOCLevelApprovalSettings WHERE UserID ='" & GBLUserID & "' and ISNULL(IsDeletedTransaction, 0) = 0 AND CompanyID = '" & GBLCompanyID & "'"
            'db.FillDataTable(embargodata, Embargo)
            ...
            '    Return "not authorized"
            'End If
            Dim jsonMainData As String = Newtonsoft.Json.JsonConvert.SerializeObject(ObjMainData)
            Dim jsonArrData As String = Newtonsoft.Json.JsonConvert.SerializeObject(Arrdata)

            ' Secure SQL Query using Parameters


            Dim sqlQuery As String = "EXEC ProcessLedgerEmbargo " & GBLCompanyID & ", " & GBLUserID & ",'" & jsonMainData & "', '" & jsonArrData & "'"

            db.ExecuteNonSQLQuery(sqlQuery)
            KeyField = "Success"
        ...
        Return KeyField
    End Function
    '---------------Open Embargo Details ---------------------------------
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(ResponseFormat:=ResponseFormat.Json)>
    Public Function GetEmbargoDetaildata(ByVal LedgerID As String) As String

        GBLCompanyID = Convert.ToString(HttpContext.Current.Session("CompanyID"))
        DBType = Convert.ToString(HttpContext.Current.Session("DBType"))
        Dim str As String = "Select nullif(ED.EmbargoStatus,'') as EmbargoStatus,nullif(ED.EmbargoReason,'') as EmbargoReason,ED.Remark as Remark,nullif(LM.LedgerName,'') as LedgerName,nullif(SP.LedgerName,'') as SalesPersonName,nullif(CBVM.BusinessVerticalName,'') as BusinessVerticalName,nullif(UM.UserName,'') as UserName from EmbargoDetails as ED Inner Join LedgerMaster as LM on LM.LedgerID = ED.LedgerID and isnull(LM.IsDeletedTransaction,0) <> 1 Inner Join ClientBusinessVerticalMaster as CBVM on CBVM.BusinessVerticalID = ED.BusinessVerticalID  and isnull(CBVM.IsDeletedTransaction,0) <> 1 Inner Join LedgerMaster as SP on SP.LedgerID = ED.SalesPersonLedgerID and isnull(SP.IsDeletedTransaction,0) <> 1 and SP.DepartmentID = -50 Inner Join UserMaster as UM on UM.UserID = ED.UserID and isnull(UM.IsDeletedUser,0) <> 1 where ED.CompanyID =  '" & GBLCompanyID & "' and ED.LedgerID = '" & LedgerID & "'  and isnull(ED.IsDeletedTransaction,0) <> 1  ORDER BY ED.EmbargoID DESC"
        db.FillDataTable(dataTable, str)
        data.Message = ConvertDataTableTojSonString(dataTable)
        Return js.Serialize(data.Message)
    End Function

    '---------------Open Placed Embargo Details ---------------------------------
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(ResponseFormat:=ResponseFormat.Json)>
    Public Function GetPlacedEmbargodata() As String

        GBLCompanyID = Convert.ToString(HttpContext.Current.Session("CompanyID"))
        DBType = Convert.ToString(HttpContext.Current.Session("DBType"))
        Dim str As String = "SELECT ED.LedgerID, ED.EmbargoID, NULLIF(ED.EmbargoReason, '') AS EmbargoReason, REPLACE(CONVERT(NVARCHAR(30), ED.CreatedDate, 106), ' ', '-') AS EmbargoDate, NULLIF(LM.LedgerName, '') AS LedgerName, ISNULL(LM.MaxCreditLimit, 0) AS MaxCreditLimit, ISNULL(LM.MaxCreditPeriod, 0) AS MaxCreditPeriod, ISNULL(LM.FixedLimit, 0) AS FixedLimit, NULLIF(UM.UserName, '') AS UserName,ISNULL(BV.BusinessVerticalNames, '') AS BusinessVerticalNames FROM EmbargoDetails ED INNER JOIN LedgerMaster LM ON LM.LedgerID = ED.LedgerID AND LM.Status = 'InActive' AND ISNULL(LM.IsDeletedTransaction, 0) <> 1 INNER JOIN UserMaster UM ON UM.UserID = ED.CreatedBy AND ISNULL(UM.isDeletedUser, 0) <> 1 LEFT JOIN ( SELECT BVD.LedgerID, STRING_AGG(CBVD.BusinessVerticalName, ', ') AS BusinessVerticalNames FROM BusinessVerticalDetails BVD INNER JOIN ClientBusinessVerticalMaster CBVD ON CBVD.BusinessVerticalID = BVD.BusinessVerticalID WHERE ISNULL(BVD.IsDeletedTransaction, 0) <> 1 GROUP BY BVD.LedgerID ) AS BV ON BV.LedgerID = ED.LedgerID WHERE ED.CompanyID = '" & GBLCompanyID & "' AND ED.EmbargoID = ( SELECT MAX(EmbargoID) FROM EmbargoDetails WHERE LedgerID = ED.LedgerID )"
        db.FillDataTable(dataTable, str)
        data.Message = ConvertDataTableTojSonString(dataTable)
        Return js.Serialize(data.Message)
    End Function
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(ResponseFormat:=ResponseFormat.Json)>
    Public Function SaveEmbargoDetailsData(ByVal EmbargoDetailsData As Object, ByVal LedgerID As String, ByVal txtStatus As String) As String
        ...
        Try
            'If db.CheckAuthories("LiftEmbargo.aspx", GBLUserID, GBLCompanyID, "CanSave") = False Then Return "You are not authorized to Save"
            'Dim Embargo = "SELECT isnull(Embargo,0) as Embargo FROM ADHOCLevelApprovalSettings WHERE UserID ='" & GBLUserID & "' and ISNULL(IsDeletedTransaction, 0) = 0 AND CompanyID = '" & GBLCompanyID & "'"
            'db.FillDataTable(embargodata, Embargo)
            ...
            If IsNumeric(EmbargoID) = False Then
                Return "fail" & EmbargoID
            End If
            Dim str As String = "UPDATE LedgerMaster SET Status = '" & txtStatus & "' WHERE CompanyID = " & GBLCompanyID & "  AND LedgerID = '" & LedgerID & "'"
            db.ExecuteNonSQLQuery(str)
            KeyField = "Success"
        Catch ex As Exception
            KeyField = "fail"
