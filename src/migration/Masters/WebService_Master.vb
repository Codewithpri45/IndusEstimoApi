...
Public Class WebService_Master
    ...
    Public Function MasterList() As String
        ...
        Else
            str = "SELECT Distinct IGM.ItemGroupID,IGM.ItemGroupName  FROM ItemGroupMaster As IGM Inner Join UserSubModuleAuthentication As UMA On UMA.ItemGroupID=IGM.ItemGroupID And UMA.CompanyID=IGM.CompanyID And UMA.CanView=1 Where IGM.IsDeletedTransaction=0 And UMA.UserID=" & GBLUserID & " Order By IGM.ItemGroupID"
        End If
    ...
    Public Function MasterGrid(ByVal masterID As String) As String
        ...
        Else
            str2 = "Select nullif(SelectQuery,'') as SelectQuery From ItemGroupMaster Where ItemGroupID='" & masterID & "' and isnull(IsDeletedTransaction,0)<>1"
        End If
    ...
    Public Function MasterGridColumnHide(ByVal masterID As String) As String
        ...
        Else
            str = "SELECT  nullif(GridColumnHide,'') as GridColumnHide,nullif(TabName,'') as TabName,nullif(ItemNameFormula,'') as ItemNameFormula,nullif(ItemDescriptionFormula,'') as ItemDescriptionFormula FROM ItemGroupMaster Where ItemGroupID= '" & masterID & "' and isnull(IsDeletedTransaction,0)<>1"
        End If
        db.FillDataTable(dataTable, str)
        data.Message = ConvertDataTableTojSonString(dataTable)
        Return js.Serialize(data.Message)
    ...
    Public Function MasterGridColumn(ByVal masterID As String) As String
        ...
        Else
            str = "select nullif(GridColumnName,'') as GridColumnName From ItemGroupMaster Where ItemGroupID='" & masterID & "' and isnull(IsDeletedTransaction,0)<>1 "
        End If
        db.FillDataTable(dataTable, str)
        data.Message = ConvertDataTableTojSonString(dataTable)
    ...
    Public Function SaveData(ByVal CostingDataItemMaster As Object, ByVal CostingDataItemDetailMaster As Object, ByVal MasterName As String, ByVal ItemGroupID As String, ByVal ActiveItem As String, ByVal StockRefCode As String) As String
        ...
        Try
            ...
            If db.GetColumnValue("CanSave", "UserSubModuleAuthentication", " UserID=" & GBLUserID & " And ItemGroupID=" & ItemGroupID) = False Then Return "You are not authorized to save..!"

            str = "Select Nullif(StockRefCode,'') as StockRefCode from ItemMaster where StockRefCode = '" & StockRefCode & "' And ItemGroupID=" & ItemGroupID & " And IsDeletedTransaction = 0"
            ...
            Else
                ItemCodestr2 = "Select nullif(ItemGroupPrefix,'') as ItemGroupPrefix from  ItemGroupMaster Where ItemGroupID=" & ItemGroupID & " and isnull(IsDeletedTransaction,0)<>1"
            End If
            db.FillDataTable(dtItemCode, ItemCodestr2)
            ItemCodePrefix = dtItemCode.Rows(0)(0)

            ItemCode = db.GeneratePrefixedNo("ItemMaster", ItemCodePrefix, "MaxItemNo", MaxItemID, "", " Where ItemCodeprefix='" & ItemCodePrefix & "' And Isnull(IsDeletedTransaction,0)=0 ")
            ...
            Else
                ...
                db.ExecuteNonSQLQuery("EXEC [UpdateItemMasterValuesMultiUnit] " & ItemID) '' To save common fields from Details to main table
                db.ExecuteNonSQLQuery("EXEC [InsertItemMasterStockValues] " & ItemID) '' Insert unit-wise stock into ItemMasterStock – Ankit, 01-06-2025
            End If
    ...
    Public Function UpdateData(ByVal CostingDataItemMaster As Object, ByVal CostingDataItemDetailMaster As Object, ByVal MasterName As String, ByVal ItemID As String, ByVal UnderGroupID As String, ByVal ActiveItem As String, ByVal StockRefCode As String) As String
        ...
        If db.GetColumnValue("CanEdit", "UserSubModuleAuthentication", " UserID=" & GBLUserID & " And ItemGroupID=" & UnderGroupID) = False Then Return "You are not authorized to update..!"

        str = "Select Nullif(StockRefCode,'') as StockRefCode from ItemMaster where StockRefCode = '" & StockRefCode & "' And ItemID <> '" & ItemID & "' And ItemGroupID=" & UnderGroupID & " And ISNULL(IsDeletedTransaction,0) = 0"
        db.FillDataTable(dt, str)
        If dt.Rows.Count > 0 AndAlso Not IsDBNull(dt.Rows(0)(0)) Then
            Return "Stock Ref. Code already exists."
        End If
        ...
        Try
            ...
            Else
                ...
                wherecndtn = "ItemID=" & ItemID & " And ItemGroupID=" & UnderGroupID & ""
                db.UpdateDatatableToDatabase(CostingDataItemMaster, TableName, AddColName, 0, wherecndtn)
                ...
                str2 = ""
                str2 = "Select nullif(ParentFieldValue,'') as ParentFieldValue From ItemMasterDetails Where ParentFieldName='ISItemActive' and FieldName='ISItemActive' and ItemID=" & ItemID & "  and ItemGroupID='" & UnderGroupID & "' and isnull(IsDeletedTransaction,0)<>1"
                Dim dtexist As New DataTable
                db.FillDataTable(dtexist, str2)
                Dim x As Integer = dtexist.Rows.Count
                If x = 0 Then
                ...
                Else
                    Dim ActiveItemUpdate As String
                    ActiveItemUpdate = "Update ItemMasterDetails Set ModifiedDate=Getdate(),UserID=" & GBLUserID & ",CompanyID=" & GBLCompanyID & ",ModifiedBy=" & GBLUserID & ",FieldValue='" & ActiveItem & "',ParentFieldValue='" & ActiveItem & "'  Where ParentFieldName='ISItemActive' and FieldName='ISItemActive' and ItemID=" & ItemID & "  and ItemGroupID='" & UnderGroupID & "' "
                    ...
                    Dim ActiveItemUpdateMaster As String
                    ActiveItemUpdateMaster = "Update ItemMaster Set ModifiedDate=Getdate(),UserID=" & GBLUserID & ",CompanyID=" & GBLCompanyID & ",ModifiedBy=" & GBLUserID & ",ISItemActive='" & ActiveItem & "'  Where ItemID=" & ItemID & "  and ItemGroupID='" & UnderGroupID & "' "
    ...
    Public Function DeleteData(ByVal txtGetGridRow As String, ByVal MasterName As String, ByVal UnderGroupID As String) As String
        ...
        Try
            ...
            Else
                str = "Update ItemMasterDetails Set DeletedBy=" & GBLUserID & ",DeletedDate=Getdate() ,IsDeletedTransaction=1  WHERE ItemID=" & txtGetGridRow & " And ItemGroupID=" & UnderGroupID & ";"
                str += "Update ItemMaster Set DeletedBy=" & GBLUserID & ",DeletedDate=Getdate() ,IsDeletedTransaction=1  WHERE ItemID=" & txtGetGridRow & " And ItemGroupID=" & UnderGroupID
            End If
    ...
    Public Function MasterGridLoadedData(ByVal masterID As String, ByVal Itemid As String) As String
        ...
        Else
            selQ = "execute SelectedRowMultiUnit  '',"
    ...
    Public Function DrillDownMasterGrid(ByVal masterID As String, ByVal TabID As String) As String
        ...
        Else
            str2 = "Select nullif(SelectQuery,'') as SelectQuery From DrilDown Where ItemGroupID='" & masterID & "' and TabName='" & TabID & "' and isnull(IsDeletedTransaction,0)<>1"
        End If
        db.FillDataTable(dt, str2)
        Dim i As Integer = dt.Rows.Count
        If i > 0 Then
    ...
    Public Function Master(ByVal masterID As String) As String
        ...
        Else
            str = "SELECT Distinct nullif(ItemGroupFieldID,'') as ItemGroupFieldID,nullif(ItemGroupID,'') as ItemGroupID,nullif(FieldName,'') as FieldName,nullif(FieldDataType,'') as FieldDataType,nullif(FieldDescription,'') as FieldDescription,nullif(IsDisplay,'') as IsDisplay,nullif(IsCalculated,'') as IsCalculated,nullif(FieldFormula,'') as FieldFormula,nullif(FieldTabIndex,'') as FieldTabIndex,nullif(FieldDrawSequence,'') as FieldDrawSequence,nullif(FieldDefaultValue,'') as FieldDefaultValue,nullif(CompanyID,'') as CompanyID,nullif(UserID,'') as UserID,nullif(ModifiedDate,'') as ModifiedDate,nullif(FYear,'') as FYear,nullif(IsActive,'') as IsActive,nullif(IsDeleted,'') as IsDeleted,nullif(FieldDisplayName,'') as FieldDisplayName,nullif(FieldType,'') as FieldType,nullif(SelectBoxQueryDB,'') as SelectBoxQueryDB,nullif(SelectBoxDefault,'') as SelectBoxDefault,nullif(ControllValidation,'') as ControllValidation,nullif(FieldFormulaString,'') as FieldFormulaString,nullif(IsRequiredFieldValidator,'') as IsRequiredFieldValidator,nullif(UnitMeasurement,'') as UnitMeasurement,IsLocked,Isnull(MinimumValue,0) AS MinimumValue,Isnull(MaximumValue,0) AS MaximumValue " &
                  " FROM ItemGroupFieldMaster Where ItemGroupID='" & masterID & "' and isnull(IsDeletedTransaction,0)<>1 Order By FieldDrawSequence "
        End If
        db.FillDataTable(dataTable, str)
    ...
    Public Function SelectBoxLoad(ByVal Qery As String, ByVal selID As String) As String
        ...
        Try
            ...
            For i = 0 To NewQuery.Length - 1
                ...
                Else
                    str3 = "Select nullif(SelectboxQueryDB,'') as SelectboxQueryDB From ItemGroupFieldMaster Where ItemGroupFieldID=" & QS & " and isnull(IsDeletedTransaction,0)<>1"
                End If
    ...
    Private Function DuplicateItemGroupValidate(ByVal TabelID As String, ByVal tblObj As Object) As Boolean
        ...
        Try
            ...
            Else
                str2 = "Select nullif(SaveAsString,'') as SaveAsString From ItemGroupMaster Where ItemGroupID='" & TabelID & "' and Isnull(IsDeletedTransaction,0)<>1"
            End If
            db.FillDataTable(dtExist, str2)
            If dtExist.Rows.Count > 0 Then
                ...
                Else
                    ...
                    For i As Integer = 0 To dt.Rows.Count - 1
                        ...
                        End If
                    Next
                    str2 = "Select Distinct ItemID From ItemMasterDetails Where ItemGroupID=" & TabelID & ColValue & BrakCount
    ...
    Public Function SaveGroupData(ByVal CostingDataGroupMaster As Object, ByVal GroupName As String, ByVal UnderGroupID As String) As String
        ...
        Try
            ...
            Else
                ...
                "from ItemSubGroupMaster where ItemSubGroupName= '" & GroupName & "' " &
            ...
            Else
                ...
                Else
                    str2 = "Select isnull(max(ItemSubGroupID),0) + 1 As ItemSubGroupID From ItemSubGroupMaster Where   Isnull(IsDeletedTransaction,0)<>1"
                End If
                ...
                Else
                    str2 = "Select isnull(ItemSubGroupLevel,0) ItemSubGroupLevel From ItemSubGroupMaster Where ItemSubGroupID = '" & UnderGroupID & "' and Isnull(IsDeletedTransaction,0)<>1"
                End If
                ...
                AddColName = ""
                AddColValue = ""
                AddColName = "ModifiedDate,CreatedDate,UserID,CompanyID,ItemSubGroupID,FYear,CreatedBy,ModifiedBy,ItemSubGroupLevel,ProductionUnitID"
                If DBType = "MYSQL" Then
                    AddColValue = "Now(),Now()," & GBLUserID & "," & GBLCompanyID & ",'" & ItemSubGroupID & "','" & GBLFYear & "'," & GBLUserID & "," & GBLUserID & ",'" & GroupLevel & "'"
                Else
                    AddColValue = "Getdate(),Getdate()," & GBLUserID & "," & GBLCompanyID & ",'" & ItemSubGroupID & "','" & GBLFYear & "'," & GBLUserID & "," & GBLUserID & ",'" & GroupLevel & "','" & ProductionUnitID & "'"
                End If
    ...
    Public Function UpdatGroupData(ByVal CostingDataGroupMaster As Object, ByVal ItemSubGroupUniqueID As String, ByVal ItemSubGroupLevel As String, ByVal GroupName As String) As String
        ...
        Try
            ...
            Else
                str2 = "select distinct nullif(ItemSubGroupName,'') as ItemSubGroupName " &
                "from ItemSubGroupMaster where ItemSubGroupName= '" & GroupName & "' AND ItemSubGroupUniqueID<>" & ItemSubGroupUniqueID & " " &
                "and isnull(IsDeletedTransaction,0)<>1"
            End If
            db.FillDataTable(dtExist, str2)
            Dim E As Integer = dtExist.Rows.Count
            ...
            Else
                ...
                Else
                    AddColName = "ModifiedDate=Getdate(),UserID=" & GBLUserID & ",CompanyID=" & GBLCompanyID & ",ModifiedBy=" & GBLUserID & ",ItemSubGroupLevel='" & ItemSubGroupLevel & "',ProductionUnitID='" & ProductionUnitID & "'"
                End If
                wherecndtn = "ItemSubGroupUniqueID=" & ItemSubGroupUniqueID & " "
    ...
    Public Function DeleteGroupMasterData(ByVal ItemSubGroupUniqueID As String) As String
        ...
        Try
            ...
            Else
                ...
                str = "Update ItemSubGroupMaster Set ModifiedBy=" & GBLUserID & ",DeletedBy=" & GBLUserID & ",DeletedDate=Getdate(),ModifiedDate=Getdate(),IsDeletedTransaction=1  WHERE ItemSubGroupUniqueID='" & ItemSubGroupUniqueID & "'"
    ...
    Public Function UpdateUserData(ByVal ItemName As Object, ByVal ItemID As String, ByVal StockRefCode As String) As String
        ...
        str = "Select Nullif(StockRefCode,'') as StockRefCode from ItemMaster where StockRefCode = '" & StockRefCode & "' And ItemID <> '" & ItemID & "' And IsDeletedTransaction = 0"
        db.FillDataTable(dt, str)
        If dt.Rows.Count > 0 AndAlso Not IsDBNull(dt.Rows(0)(0)) Then
            Return "Stock Ref. Code already exists."
        End If
        Try
            TableName = "ItemMaster"
            AddColName = "ModifiedDate=Getdate(),UserID=" & GBLUserID & ",ModifiedBy=" & GBLUserID & " "
            wherecndtn = "ItemID='" & ItemID & "' "
